 ...
 */
class CRM_Sktokens_Upgrader extends CRM_Extension_Upgrader_Base {
  ...
  public function upgrade_1000(): bool {
    $this->ctx->log->info('Improve SK Token performance');
    $tokenDisplays = \Civi\Api4\SearchDisplay::get(FALSE)
      ->addSelect('id', 'settings')
      ->addWhere('type', '=', 'tokens')
      ->execute()
      ->indexBy('id');
    $itemsToRemove = ['style', 'limit', 'sort', 'pager'];
    foreach ($tokenDisplays as $id => $tokenDisplay) {
      $changed = FALSE;
      $settingsArray = $tokenDisplay['settings'];
      foreach ($itemsToRemove as $item) {
        if (isset($settingsArray[$item])) {
          $changed = TRUE;
          unset($settingsArray[$item]);
        }
      }
      if ($changed) {
        \Civi\Api4\SearchDisplay::update(FALSE)
          ->addValue('settings', $settingsArray)
          ->addWhere('id', '=', $id)
          ->execute();
      }
    }
